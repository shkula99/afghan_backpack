...
import 'package:untitled1/models/historical_place_photo.dart';
...
class HistoricalPlaceScreenWidget extends StatelessWidget {
  ...
  const HistoricalPlaceScreenWidget({super.key, required this.historicalPlaceId});

  Future<HistoricalPlace> _fetchHistoricalPlace(BuildContext context) async {
    final dbHelper = DatabaseHelper();
    final historicalPlace = await dbHelper.fetchHistoricalPlaceById(historicalPlaceId);
    return historicalPlace!;
  }

  Future<List<HistoricalPlacePhoto>> _fetchHistoricalPlacePhotos(
      BuildContext context,
      ) async {
    final dbHelper = DatabaseHelper();
    final photos = await dbHelper.fetchHistoricalPlacePhotos(historicalPlaceId);
    return photos;
  }
  ...
  Widget _buildPhotoGallery(
      BuildContext context,
      List<HistoricalPlacePhoto> photos,
      ) {
    return SizedBox(
      height:
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<HistoricalPlace>(
      future: _fetchHistoricalPlace(context), // Fetching historical place data from DB
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData) {
          return const Center(child: Text('No historical place data found.'));
        }

        final historicalPlace = snapshot.data!;
                ...
                Stack(
                  children: [
                    ClipRRect(
                      ...
                      child: Image.asset(
                        historicalPlace.image, // Assuming the image is fetched from a URL or network path
                        width: double.infinity,
                        height: 300,
                        fit: BoxFit.cover,
                      ),
                      ...
                      Row(
                        children: [
                          IconButton(
                            ...
                            onPressed: () => _openLocation(context, historicalPlace.latitude, historicalPlace.longitude),
                          ),
                          const SizedBox(width: 4),
                          Text(
                            '${provinceNames[historicalPlace.provinceId] ?? "Unknown"}, AFG',
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      FutureBuilder<List<HistoricalPlacePhoto>>(
                        future: _fetchHistoricalPlacePhotos(context),
                        builder: (context, snapshot) {
