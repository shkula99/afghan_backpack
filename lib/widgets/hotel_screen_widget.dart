...
import 'package:untitled1/models/hotel_photo.dart';
...
class HotelScreenWidget extends StatelessWidget {
  ...
  const HotelScreenWidget({super.key, required this.hotelId});

  Future<Hotel> _fetchHotel(BuildContext context) async {
    final dbHelper = DatabaseHelper();
    final hotel = await dbHelper.fetchHotelById(hotelId);
    return hotel!;
  }

  Future<List<HotelPhoto>> _fetchHotelPhotos(
      BuildContext context,
      ) async {
    final dbHelper = DatabaseHelper();
    final photos = await dbHelper.fetchHotelPhotos(hotelId);
    return photos;
  }
  ...
  // Contact icons
  Widget _buildContactIcons(BuildContext context, Hotel hotel) {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<Hotel>(
      future: _fetchHotel(context), // Fetching restaurant data from DB
      builder: (context, snapshot) {
        ...
        return Scaffold(
                ...
                Stack(
                  children: [
                    ClipRRect(
                      ...
                      child: Image.asset(
                            hotel.image, // Assuming the image is fetched from a URL or network path
                        width: double.infinity,
                        height: 300,
                        fit: BoxFit.cover,
                      ),
                      ...
                      Row(
                        children: [
                          IconButton(
                            ...
                            onPressed: () => _openLocation(context, hotel.latitude, hotel.longitude),
                          ),
                          const SizedBox(width: 4),
                          Text(
                            '${provinceNames[hotel.provinceId] ?? "Unknown"}, AFG',
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      FutureBuilder<List<HotelPhoto>>(
                        future: _fetchHotelPhotos(context),
                        builder: (context, snapshot) {
          ...
          bottomNavigationBar: Padding(
            ...
            child: _buildContactIcons(context, hotel),
          ),
        );
      },
    );
